...
describe("manual upload Express assembly", () => {
  ...
  it("denies upload assembly when requester is not the configured owner", async () => {
    const built = await buildManualUploadRequestFromMultipart({
      ownerEmail: "owner@example.com",
    ...
    assert.equal(built.ok, false);
    if (built.ok) return;
    assert.equal(built.statusCode, 403);
    assert.ok(built.errors.some((error) => error.code === "OWNER_ACCESS_DENIED"));
  });
