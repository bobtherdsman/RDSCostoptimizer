...
## 2. Bounded, Low-Impact Collector
The collector must be safe to run against a live RDS SQL Server instance. Workload evidence must come from lightweight, read-only SQL Server DMV, catalog, and performance-counter queries. The approved SSAT-style flow may use bounded collector-owned staging tables and a SQL Agent sampling job that are removed after export; it must never modify customer application tables or data. It must not use tracing, Extended Events, Query Store scraping, query-plan capture, workload replay, application-table scans, or customer data extraction.
...
Uploads may contain one server or multiple servers. Every recommendation, blocker, and top database must be correct per server before any fleet-level summary is shown.
...
Every recommendation must respect SQL Server edition limits, EE-to-SE eligibility, Optimize CPU support, SQL Server-visible processor metadata, and RDS orderability.

## 10. Evidence Over Opinion
Customer-facing claims must come from verified AWS/Microsoft facts or harness-validated logic. Anything unproven remains internal and marked for verification.

## 11. Explain Blockers Clearly
If memory, IOPS, throughput, tempdb, edition, orderability, or evidence quality blocks optimization, the report must explain the blocker and the next investigation or tuning action.

## 12. Harness Before Report
No recommendation should be shown unless it passes the cost harness: CPU, memory, IOPS, throughput, tempdb, edition, orderability, evidence quality, and independence from SSATWeb sizing logic.
The harness verifies production output and guards against regressions; it must not choose the target size or supply production formulas at runtime.
